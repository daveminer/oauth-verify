...
} from "../../contracts/oauth_verify/src/artifacts/EasyPrivateVoting.js";
import {
  AccountManager,
  AccountWallet,
  CompleteAddress,
...
describe("Accounts", () => {
  ...
  it("Calls entrypoint", async () => {
    const tx = await EasyPrivateVotingContract.deploy(wallets[0], accounts[0])
      .send()
      .deployed();
    await tx.entrypoint().send().wait();
  });
  ...
  it("Creates accounts with fee juice", async () => {
    ...
    await EasyPrivateVotingContract.deploy(wallets[0], accounts[0])
      .send()
  ...
  it("Deploys first unfunded account from first funded account", async () => {
    const tx_acc = await randomAccountManagers[0].deploy({
  ...
  it("Sponsored contract deployment", async () => {
    ...
    await Promise.all(
      accounts.map((a) =>
        a.deploy({ fee: { paymentMethod: sponsoredPaymentMethod } })
      )
    );
