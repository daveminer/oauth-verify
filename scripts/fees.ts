...
import { EasyPrivateVotingContract } from "../contracts/oauth_verify/src/artifacts/EasyPrivateVoting.js";
import { TokenContract } from "@aztec/noir-contracts.js/Token";
// TODO: replace with import from aztec.js when published
import { SponsoredFeePaymentMethod } from "@aztec/aztec.js/fee/testing";
import { getDeployedSponsoredFPCAddress } from "../contracts/oauth_verify/src/utils/sponsored_fpc.js";
...
async function main() {
  ...
  const feeJuice = await FeeJuiceContract.at(
    nodeInfo.protocolContractAddresses.feeJuice,
    wallets[0]
  );
  ...
  const votingContract = await EasyPrivateVotingContract.deploy(
    ...
    .send()
    .deployed();
  const bananaCoin = await TokenContract.deploy(
    wallets[0],
    wallets[0].getAddress(),
    "bananaCoin",
    "BNC",
  ...
  logger.info(
    `New account at ${newWallet.getAddress()} deployed using claimed funds for fees.`
  );
  ...
  await votingContract
    ...
    .methods.cast_vote(wallets[0].getAddress())
    .send({ fee: { paymentMethod: useFeeJuice } })
    .wait();
  ...
  const fpc = await FPCContract.deploy(
    ...
    .send()
    .deployed();
  const fpcClaim = await feeJuicePortalManager.bridgeTokensPublic(
    fpc.address,
    FEE_FUNDING_FOR_TESTER_ACCOUNT,
    true
  );
  ...
  await bananaCoin.methods
    .mint_to_public(newWallet.getAddress(), FEE_FUNDING_FOR_TESTER_ACCOUNT)
    .send()
    .wait();
  const bananaBalance = await bananaCoin
    .withWallet(newWallet)
    .methods.balance_of_private(newWallet.getAddress())
    .simulate();
  ...
  await feeJuice.methods
    .claim(
      fpc.address,
      fpcClaim.claimAmount,
      fpcClaim.claimSecret,
      fpcClaim.messageLeafIndex
    )
    .send()
    .wait();

  logger.info(
    `Fpc fee juice balance ${await feeJuice.methods
  ...
  await bananaCoin
    ...
    .send({ fee: { paymentMethod: privateFee } })
    .wait();
  ...
  await bananaCoin
    ...
    .send({ fee: { paymentMethod: publicFee } })
    .wait();
